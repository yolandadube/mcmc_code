...
mcmcParameters2  = ['A', 'k0', 'n', 'm', 'Delta']
#--------------------------------------------------------------------------------------------------------------------------
...
# load MCMC data file
file1       = np.loadtxt(filePath1+'MCMC_Model2_meerKATLBandmeerKATLBand_0.001.dat')
file2       = np.loadtxt(filePath1+'MCMC_Model2_meerKATUHFBandmeerKATUHFBand_0.001.dat')
file3       = np.loadtxt(filePath1+'MCMC_Model2_skaOIMBand1skaOIMBand1_0.001.dat')
#file4       = np.loadtxt(filePath+'MCMC_skaOIMBand1skaOIMBand1_fg0_005h_beam.dat')
#--------------------------------------------------------------

# axis labels for triangular plot
axesNames   = ["x%s"%i for i in range(len(mcmcParameters2))]
...
for i in range(len(mcmcParameters2)):
    for j in range(i, len(mcmcParameters2)):
        # axes 
        ax = g.subplots[j,i]
        # Check if the subplot corresponds to k0 on the x-axis
        if mcmcParameters2[i] == 'k0':
...
# saving as pdf
g.export(f'{filePath}MCMCcontourNewPlots21.pdf')
#--------------------------------------------------------------------------------------------------------------------------