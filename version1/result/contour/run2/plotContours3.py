...
filePath    = f"{mainPath}result/contour/run2/"
filePath1    = f"{mainPath}result/"
filePath2    = f"{mainPath}functions/ResidualData/"
#---#
...
# load MCMC data file
file1       = np.loadtxt(filePath2+'MCMC_HAlphaHAlpha_0.001.dat')
file2       = np.loadtxt(filePath2+'MCMC_desiELGdesiELG_0.001.dat')
file3       = np.loadtxt(filePath2+'MCMC_desiBGSdesiBGS_0.001.dat')
...
cases      = [samplesMCMC3, samplesMCMC2, samplesMCMC1]
colors     = [sns.xkcd_rgb["purple"], sns.xkcd_rgb["green"], sns.xkcd_rgb["orange"]]
linestyles = ['-', '-', '-', '-']
#labelS     = [r'$\mathrm{SKAO\;Band}\;1$', r'$\mathrm{HIRAX\;256}$']
labelS     = [r'$\mathrm{DESI\ BGS}$', r'$\mathrm{Euclid}$', r'$\mathrm{DESI\ ELG}$']
...
for i in range(len(mcmcParameters)):
    for j in range(i, len(mcmcParameters)):
        ...
        if mcmcParameters[i] == 'k0':
            # insert vertical line at k0 = 0.0163 h/Mpc
            ax.vlines(0.0168, ax.get_ylim()[0], ax.get_ylim()[1], color='grey', 
...
# saving as pdf
g.export(f'{filePath}MCMCcontourGalaxyPlots.pdf')
#--------------------------------------------------------------------------------------------------------------------------