...
filePath    = f"{mainPath}result/contour/run3/"
#---#

# load initial guess data file for MCMC parameters
#guessFile   = np.loadtxt(filePath+'MCMC_guess.dat')

# load MCMC data file
file1       = np.loadtxt(filePath+'MCMC_HAlphaHAlpha.dat')
file2       = np.loadtxt(filePath+'MCMC_desiELGdesiELG.dat')
file3       = np.loadtxt(filePath+'MCMC_skaOIMBand1skaOIMBand1.dat')
file4       = np.loadtxt(filePath+'MCMC_hirax256hirax256.dat')
#--------------------------------------------------------------
...
cases      = [samplesMCMC2, samplesMCMC1, samplesMCMC3, samplesMCMC4]
colors     = [sns.xkcd_rgb["blue"], sns.xkcd_rgb["red"], sns.xkcd_rgb["green"], sns.xkcd_rgb["purple"]]
linestyles = ['-', '-', '-', '-']
labelS     = [r'$\mathrm{DESI\;ELG}$', r'$\mathrm{Stage\;IV\;H}\alpha$', r'$\mathrm{SKAO\;Band}\;1$', r'$\mathrm{HIRAX\;256}$']
...
# saving as pdf
g.export(f'{filePath}MCMCcontour.pdf')
#--------------------------------------------------------------------------------------------------------------------------