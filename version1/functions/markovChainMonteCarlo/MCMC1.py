...
def mcmcAlgorithm1(mainPath, mcmcParameters1, priors1, chains, walks, cosmologY, gauge, nonLinear, k, k_NL0, k_fg, \
    ...
    from guess1 import initialGuess1
    from latexFonts1 import latexDictionary1
    from probability1 import logProbability1
    #---#
    ...
    mcmcGuess      = initialGuess1(mainPath, cosmologY, k, PData[0])
    ...
    #models   = np.array(mcmcGuess) + (1.0e4*np.random.randn(chains, len(mcmcParameters1)))
    step_sizes = np.array([2e5, 1e-3, 0.2, 2.0, 0.2])  # Step sizes for [A, k0, n, m, beta]
    models = np.array(mcmcGuess) + step_sizes * np.random.randn(chains, len(mcmcParameters1))

    
    with mp.Pool() as pool:
        sampler  = emcee.EnsembleSampler(chains, len(mcmcParameters1), logProbability1, pool=pool, \
                                         args=(mainPath, k, PData, priors1))
    ...
    fig, axes = pl.subplots(len(mcmcParameters1), sharex=True)
    ...
    dataFile2 = open(filePath + f"MCMC_Model1_{specsSurvey1}{specsSurvey2}_{file_suffix}.dat", 'w')
